...
namespace _3DGame
{
    public static class Collider
    {
        ...
        {
            bool result = false;
            character.CreateBoundingSphere(CharacterBoundingSphere_Scale,
                CharacterBoundingSphere_XOffset,
                CharacterBoundingSphere_YOffset,
                CharacterBoundingSphere_ZOffset);
        ...
        public static BoundingSphere CreateBoundingSphere(this IGameObject gameObject, float scale)
        {
            ...
            for (int i = 0; i < gameObject.Model.Meshes.Count; i++)
            {
                ...
                gameObject.BoundingSphere = BoundingSphere.CreateMerged(gameObject.BoundingSphere,
                    currentMeshBoundingSphere.Transform(Matrix.CreateScale(scale) 
                    * gameObject.Transforms[i]
                    * gameObject.World));
            }
        ...
        public static BoundingSphere CreateBoundingSphere(this IGameObject gameObject, float scale, float xOffset ,float yOffset, float zOffset)
        {

            gameObject.BoundingSphere =
                gameObject.Model.Meshes[0].BoundingSphere.Transform(Matrix.CreateScale(scale)
                * gameObject.Transforms[0] 
                * gameObject.World
                );
            ...
            for (int i = 0; i < gameObject.Model.Meshes.Count; i++)
            {
                ...
                gameObject.BoundingSphere = BoundingSphere.CreateMerged(gameObject.BoundingSphere,
                    currentMeshBoundingSphere.Transform(Matrix.CreateScale(scale)
                        *gameObject.Transforms[i] 
                    * gameObject.RotationMatrix 
                    * Matrix.CreateTranslation(gameObject.Position + new Vector3(xOffset,yOffset,zOffset))
